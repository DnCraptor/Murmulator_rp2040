...
bool load_image_sna(char *file_name){
	...
	if (sd_file_size(&sd_file)<SNA_48K_SIZE){
		sd_close_file(&sd_file);return false;
	}
	if (sd_file_size(&sd_file)>SNA_128K_SIZE2){
		sd_close_file(&sd_file);return false;
	} 
	...
	if (sd_file_size(&sd_file) == SNA_48K_SIZE){
		...
		//readCPUstate(header);
		//printf("*V48\n");
	} else if (sd_file_size(&sd_file) > SNA_48K_SIZE){
		im_hw = 128;
		//printf("*V128\n");
	} else {
		// Invalid
	...
	if (im_hw==48){
		...
		file_descr = sd_seek_file(&sd_file,sd_file_pos(&sd_file)-ZX_RAM_PAGE_SIZE-4);
		if (file_descr != FR_OK){sd_close_file(&sd_file);return false;}
		pageSize=ZX_RAM_PAGE_SIZE;
		bufferIn = (uint8_t*)&sd_buffer;
		bufferOut = &RAM[zx_RAM_bank_active*ZX_RAM_PAGE_SIZE];
		...
		*/
		...
		file_descr = sd_seek_file(&sd_file,sd_file_pos(&sd_file)+4);
		if (file_descr != FR_OK){sd_close_file(&sd_file);return false;}
