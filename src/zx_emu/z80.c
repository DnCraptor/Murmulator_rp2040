...
static inline uint16_t rw(z80* const z, uint16_t addr) {
	return (z->read_byte(z->userdata, addr + 1) << 8) | z->read_byte(z->userdata, addr);
}
...
static void in_r_c(z80* const z, uint8_t* r) {
	z->_hi_addr_port=z->b;//add addr Hi
	*r = z->port_in(z, z->c);
	z->zf = *r == 0;
	z->sf = *r >> 7;
	z->pf = parity(*r);
...
static void outi(z80* const z) {
	z->_hi_addr_port=z->b;//-1;//add_addr_h
...
static inline uint16_t displace(z80* const z, uint16_t base_addr, int8_t displacement) {
...
void z80_debug_output(z80* const z) {
	#ifdef Z80_DEBUG
	printf("PC:%04X AF:%04X BC:%04X DE:%04X HL:%04X SP:%04X (%02X %02X [%02X %02X] %02X %02X) IX:%04X IY:%04X I:%02X R:%02X\n",
		z->pc, (z->a << 8) | get_f(z), get_bc(z), get_de(z), get_hl(z),
		z->sp, rb(z, z->sp-2), rb(z, z->sp-1), rb(z, z->sp), rb(z, z->sp+1), rb(z, z->sp+2), rb(z, z->sp+3),
		z->ix, z->iy, z->i, z->r);
/*
...
*/
	...
	printf("AF_:%02X%02X BC_:%02X%02X DE_:%02X%02X HL_:%02X%02X\n",z->a_, z->f_, z->b_, z->c_, z->d_, z->e_, z->h_, z->l_);
	#endif
}
...
void FAST_FUNC(exec_opcode)(z80* const z, uint8_t opcode) {
	...
	const uint8_t a = z->a;
	z->_hi_addr_port=z->a;//add addr Hi
	z->a = z->port_in(z, port);
	z->mem_ptr = (a << 8) | (z->a + 1);
	//z->mem_ptr = (port + 1) | (a << 8);//
...
void exec_opcode_ed(z80* const z, uint8_t opcode) {
	...
	case 0x41: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->b); break; // out (c), b
	case 0x49: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->c); break; // out (c), c
	case 0x51: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->d); break; // out (c), d
	case 0x59: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->e); break; // out (c), e
	case 0x61: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->h); break; // out (c), h
	case 0x69: z->_hi_addr_port=z->b; z->port_out(z, z->c, z->l); break; // out (c), l
	case 0x71: z->_hi_addr_port=z->b; z->port_out(z, z->c, 0); break; // out (c), 0
	case 0x79:
	z->_hi_addr_port=z->b;
