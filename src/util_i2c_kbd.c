...
bool i2c_decode_kbd(){
	...
	if ((scan != oldscan)) {
		...
		oldscan = scan;
		pressed_timeout = my_millis();
		return true;
	}
	if((scan>0)&&(pressed_timeout>0)&&((my_millis()-pressed_timeout)>I2C_KEY_PRESSED_TIMEOUT)){
		pressed_timeout = 0;
		repeat_timeout = my_millis();
		translate_keys(i2c_data);
		return true;
	}
	if((scan>0)&&(repeat_timeout>0)&&((my_millis()-repeat_timeout)>I2C_KEY_REPEAT_TIMEOUT)){
		repeat_timeout = my_millis();
		translate_keys(i2c_data);
		return true;
	}
