...
static uint32_t conv_colorNORM[2][16]; //2к
static uint32_t conv_colorINV[2][16];	//2к
...
void graphics_set_mode(tv_out_mode_t mode){
	...
	{
		case _624_lines:
			v_mode.N_lines=312;break;
		case _625_lines:
			v_mode.N_lines=625;break;
		case _524_lines:
			v_mode.N_lines=262;break;
		case _525_lines:
			v_mode.N_lines=525;break;	
	}
	...
	switch (tv_out_mode.c_freq)
		{
			case _3579545:color_freq=3.579545*1e6;break;
			case _4433619:color_freq=4.43361875*1e6;break;
		}

	v_mode.H_len=((color_freq*4)/1e6)*63.9;
    v_mode.H_len&=0xfffffff8;

	v_mode.sync_size=4.7*v_mode.H_len/64;
	v_mode.sync_size&=0xfffffff8;

	v_mode.begin_img_shx=10.5*v_mode.H_len/64;
	v_mode.img_W=v_mode.H_len-((12*v_mode.H_len)/64);
	v_mode.img_W&=0xfffffffc;
		
	v_mode.LVL_C_MAX	= 15;
	v_mode.SYNC_TMPL=0;
	v_mode.NO_SYNC_TMPL=CONV_DAC(v_mode.LVL_C_MAX)|(1<<SYNC_PIN);
	v_mode.LVL_BLACK=0+v_mode.LVL_C_MAX;
	...
	v_mode.LVL_BLACK_TMPL=CONV_DAC(v_mode.LVL_BLACK)|(1<<SYNC_PIN);
...
void graphics_set_palette(uint8_t i, uint32_t color888){
		...
		uint8_t base8=v_mode.LVL_BLACK;
		
		const int cycle_size=4; 
		int8_t Y8=((int)(Y*v_mode.LVL_Y_MAX))+base8;
		...
		case g_TV_OUT_PAL:
			...
			if (tv_out_mode.cb_sync_PI_shift_lines) 
			 ...
			 for(int i=0;i<cycle_size;i++)
        		{       
					...
					float k=1.3*tv_out_mode.color_index;//подобрать , чтобы не было перегруза 1.25 или увеличить для более ярких цветов
					int max_v=v_mode.LVL_C_MAX;	
			...
			uint8_t ampl=0;
			uint8_t max_ampl=v_mode.LVL_C_MAX;
...
static void __not_in_flash_func(main_video_loop)(){
	...
	while(dma_inx_out!=dma_inx){
			...
			if (tv_out_mode.tv_system==g_TV_OUT_PAL) 
				{
					...
					if (tv_out_mode.cb_sync_PI_shift_lines)	
						{
							...
							{
								if ((line_active==0)||
									((line_active==v_mode.N_lines/2)&&((tv_out_mode.N_lines==_625_lines)||(tv_out_mode.N_lines==_525_lines)))) 
														 {
															dec_str+=2;
															static bool is_inv;
															if (is_inv)
					...
					else
						{
 							if (tv_out_mode.cb_sync_PI_shift_half_frame)
							{
								if ((line_active==0))  {g_str_index+=2;dec_str+=2;}//нейтрализация сдвига фазы "лишней строки"(не кратной 4)
								if ((tv_out_mode.N_lines==_625_lines)||(tv_out_mode.N_lines==_525_lines))
									if (line_active==v_mode.N_lines/2)  {g_str_index+=2;dec_str+=2;}
							}
					...
					if (tv_out_mode.cb_sync_PI_shift_half_frame)
					 {
						 if ((line_active==0))  {dec_str^=2; g_str_index-=1;}//нейтрализация сдвига фазы "лишней строки"(не кратной 4)
						 if ((tv_out_mode.N_lines==_625_lines)||(tv_out_mode.N_lines==_525_lines))
					 	 	if (line_active==v_mode.N_lines/2)  {dec_str^=2; g_str_index-=1;}
					 }
					 li=(g_str_index)&1;
		...
		if (is_line_visible){
			...
			if (li)	memcpy(out_buf8+0+mul_sh*4,cb[1],40);
			else memcpy(out_buf8+0+mul_sh*4,cb[0],40);
			//цветовая вспышка V2
		   ...
		   switch (tv_out_mode.N_lines){
				...
				case _625_lines:
					  ...
					  buf_sh=(tv_out_mode.c_freq==_4433619)?72:60;
					  if ((line_active>4)&&(line_active<310)) {line_inx=line_active-23;};//-23
           			  if ((line_active>317)&&(line_active<622)) {line_inx=line_active-335;};//-335
					  line_inx-=24;
					break;
				case _524_lines:
				case _525_lines:
			...
			if ((line_inx>=240)||(line_inx<0)||(vbuf==NULL)){
				//вне изображения
				memset(out_buf8,v_mode.LVL_BLACK_TMPL,v_mode.img_W);
			} else {
				//зона изображения
				...
				out_buf32+=v_mode.begin_img_shx/4;
				if (tv_out_mode.mode_bpp==g_mode_320x240x4bpp){
								...
								*out_buf8++=c_4[0];
								...
								if (next_ibuf<=0){
									if(is_read_buf) {
										c8=*(++vbuf8);
										#ifndef SOFT_CVBS_NO_HUD
										if(ovlbuf8!=NULL){if((*ovlbuf8)<0x10){c8=(*ovlbuf8&0x0f);};*ovlbuf8++;}
										#endif
										cout32=conv_color[li][c8&0xf];
									} else {
										#ifndef SOFT_CVBS_NO_HUD
										if(ovlbuf8!=NULL){if((*ovlbuf8)<0x10){c8=(*ovlbuf8<<4);};*ovlbuf8++;}
										#endif
										cout32=conv_color[li][c8>>4];
									}
									next_ibuf+=0x100;
					...
					uint8_t* vbuf8=vbuf+(line_inx)*g_buf.width;
		...
		transfer_count_DMA_CTRL[dma_inx_out]=v_mode.H_len-dec_str;
...
static bool __not_in_flash_func (video_timer_callback(repeating_timer_t *rt)){
	main_video_loop();
	return true;
}

tv_out_mode_t graphics_get_default_mode(g_out video_out){
	...
	if((video_out==g_out_AUTO)||(video_out==g_out_PAL)){
		tv_mode.tv_system=g_TV_OUT_PAL;
		tv_mode.N_lines=_625_lines;
		tv_mode.c_freq=_4433619;
	}
	return tv_mode;
};
...
void graphics_init(){
	...
	{
		transfer_count_DMA_CTRL[i]=v_mode.H_len/1;
	}
