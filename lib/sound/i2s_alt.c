...
void i2s_init()
{
    ...
    dma_i2s=dma_claim_unused_channel(true);
    ...
    //dma_channel_set_trans_count (dma_i2s, 1<<31, true);
	...
	// 	dma_i2s_ctrl,
	// 	&cfg_dma,
	// 	&dma_hw->ch[dma_i2s].transfer_count,	// Write address
...
inline void i2s_out(int16_t l_out,int16_t r_out)
                        {
                            i2s_data=(((uint16_t)l_out)<<16)|(((uint16_t)r_out));
