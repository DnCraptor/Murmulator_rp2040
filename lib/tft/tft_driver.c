...
//alignas(1024) static uint32_t conv_arr[256];
alignas(512) static uint16_t conv_arr[512]; 
static uint32_t* conv_2pix_4_to_16;

extern uint8_t graph_buf[];

static uint8_t* v_buf=graph_buf;
...
static void ST7789v_begin(void){
...
static void ILI9341_begin(bool V) {
...
void graphics_init(g_out g_out,fr_rate rate){
	...
	conv_2pix_4_to_16=(uint32_t*)conv_arr;
	...
	sleep_ms(20);
	if((g_out==g_out_TFT_ILI9341){
		ILI9341_begin(false);
	}else if(g_out==g_out_TFT_ILI9341V)){
		ILI9341_begin(true);
	}
	#ifdef TFT_ST7789v
		ST7789v_begin();
